  ...
  index.scrollComments=function(time){
    ...
    if ($("#citycommentindex ul").attr("name") != "hovered") {
      ...
      if (isIE6) {
        ...
        //所以在ie6中实行透明的禁用。
        $("#citycommentindex li:last").css({"height": 0});
      } else {
        $("#citycommentindex li:last").css({"opacity": 0, "height": 0});
        //列表最后的li透明和高度设置为0
      }
      ...
      $("#citycommentindex li:first").animate({"height": height}, "slow");
      //列表顶部的li高度逐渐变高以把下面的li推下去
      if (!isIE6) {
        $("#citycommentindex li:first").animate({"opacity": "1"}, "slow");
        //在非IE6浏览器中设置透明淡入效果
      }
    }
    ...
    $(window).scroll(function () {
      ...
      if (winTop > 0) {
        if ((typeof ActiveXObject != "undefined") && (typeof XMLHttpRequest == "undefined")) {
          ...
          $(".ind_slogan2").css({"position": "fixed", "top":objTopHeight});
        }
